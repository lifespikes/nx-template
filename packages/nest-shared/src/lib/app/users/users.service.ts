...
import { UserCreatedEvent } from '@spikey/nest-shared/app/users/events/user-created.event';
import { type ExtendedPrismaClient } from '@spikey/nest-shared/prisma/extended-prisma-client';
import { CustomPrismaService } from 'nestjs-prisma';
import { HashService } from '@spikey/nest-shared/app/auth/hash.service';
import { UserType } from '@spikey/shared/types';
import { getAllPermissions } from '@spikey/shared/permissions';
...
export class UsersService {
  ...
  async user(id: number) {
    ...
    return {
      ...user,
      permissions: user?.role ? getAllPermissions(user?.role) : []
    } as UserType;
  }
